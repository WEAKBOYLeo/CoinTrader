...
class TestLegacyRefetchBudgetRemoved:
    """旧「每 60s 窗口 N 个 symbol」固定预算已删除（实施计划书 v2.0 T2）。

    限流预算现由共享 RateLimitCoordinator 按 scope/优先级控制
    （tests/test_rate_scheduler.py）；策略层一次性刷新全部到期候选。
    """
    ...
    def _ten_symbol_env(self, tmp_path):
        ...
        cfg = _dyn_config(candidate_pool_max_symbols=10)
        strat, _ = make_strategy(tmp_path, data, config=cfg, now_fn=lambda: clock["t"])
        return strat, data, clock

    def test_all_due_symbols_refreshed_in_one_call(self, tmp_path):
        strat, data, _clock = self._ten_symbol_env(tmp_path)
        strat.refresh_candidates()
        assert data.funding_calls == 10  # 无固定 symbol 数预算
    ...
    def test_fresh_cache_not_refetched_before_boundary(self, tmp_path):
        ...
        strat.refresh_candidates()
        assert data.funding_calls == 10
        clock["t"] = NOW + 8 * 3600 + 1000  # 跨过结算周期：重拉
        strat.refresh_candidates()
        assert data.funding_calls > 10
